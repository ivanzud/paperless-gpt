...
| `VISION_OLLAMA_THINK`               | (Ollama vision OCR only) Override `OLLAMA_THINK` for the vision model. Set to `false` to disable reasoning or `true` to enable it; when unset, the vision model inherits `OLLAMA_THINK` and then the model default. | No       |                            |
