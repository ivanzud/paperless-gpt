...
func (p *recordingOCRProvider) ProcessImage(_ context.Context, _ []byte, pageNumber int) (*ocr.OCRResult, error) {
	...
	// Long enough to clear IsMeaningfulOCRText's 12-alphanumeric floor. A bare
	// "page N" is rejected as non-actionable, which would fail this test for a
	// reason that has nothing to do with the page limit it actually covers.
	return &ocr.OCRResult{Text: fmt.Sprintf("page %d scanned document text", pageNumber)}, nil
}
...
func TestUploadProcessedPDF(t *testing.T) {
	...
	env.setMockResponse("/api/documents/post_document/", func(w http.ResponseWriter, r *http.Request) {
		...
		// Return a task ID
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(fmt.Sprintf("\"%s\"", mockTaskID)))
	})
