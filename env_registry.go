...
var envRegistry = []EnvVar{
	...
	{Name: "AUTO_OCR_THEN_CLASSIFY", Category: "Tags & automation", Secret: false, Default: "false", Description: "Run metadata classification immediately after successful auto OCR, using the improved OCR text."},
	...
	{Name: "AUTO_TAG_COMPLETE", Category: "Tags & automation", Secret: false, Default: "paperless-gpt-auto-complete", Description: "Tag added to documents after auto-processing is complete. Only applied during auto-processing, not manual review. Set to an empty string (`AUTO_TAG_COMPLETE=\"\"`) to disable. When the variable is unset, the default tag is used. The tag is created automatically in paperless-ngx at startup if it does not exist."},
	...
	{Name: "BACKGROUND_DOCUMENT_MAX_FAILURES", Category: "Processing & limits", Secret: false, Default: "", Description: "Deprecated alias for `OCR_MAX_RETRIES`; used only when `OCR_MAX_RETRIES` is unset."},
	{Name: "BACKGROUND_DOCUMENT_TIMEOUT", Category: "Processing & limits", Secret: false, Default: "15m", Description: "Maximum time allowed for one background document. Set to `0` to disable the per-document timeout."},
	...
	{Name: "CORRESPONDENT_PROMPT_LIMIT", Category: "Processing & limits", Secret: false, Default: "0", Description: "Maximum number of existing correspondents embedded into the correspondent suggestion prompt. Names occurring in the document are preferred. `0` (default) sends the full list. Useful for large installations and local LLMs with small context windows, where hundreds of correspondents make prompts slow or overflow the context."},
	...
	{Name: "LLM_TEMPERATURE", Category: "LLM", Secret: false, Default: "0", Description: "(Ollama metadata only) Sampling temperature for title, tag, and metadata generation. When unset, the native-client baseline is `0`. Must be a non-negative finite number; invalid values are ignored with a warning. Does not apply to Vision OCR or other LLM providers."},
	...
	{Name: "MISTRAL_OCR_IMAGE_LIMIT", Category: "OCR", Secret: false, Default: "", Description: "Max images Mistral OCR extracts per document when OCR_PROVIDER is mistral_ocr. Unset uses the API default."},
	{Name: "MISTRAL_OCR_IMAGE_MIN_SIZE", Category: "OCR", Secret: false, Default: "", Description: "Min height/width (px) for a region to be extracted as an image rather than transcribed, when OCR_PROVIDER is mistral_ocr. Raise this to stop small boxed fields (e.g. handwritten form entries) from being skipped as images."},
	...
	{Name: "OCR_FAILED_TAG", Category: "OCR", Secret: false, Default: "", Description: "Deprecated alias for `FAIL_TAG`; used only when `FAIL_TAG` is unset."},
	...
	{Name: "OCR_PROVIDER", Category: "OCR", Secret: false, Default: "llm", Description: "OCR provider to use (`llm`, `azure`, `google_docai`, or `mistral_ocr`)."},
	{Name: "OCR_SKIP_FAILED_DOCUMENTS", Category: "OCR", Secret: false, Default: "false", Description: "Remove an auto-OCR document from the queue and apply `FAIL_TAG` immediately after an OCR failure."},
	{Name: "OCR_SKIP_FAILED_PAGES", Category: "OCR", Secret: false, Default: "false", Description: "Continue OCR after an individual page fails; the document fails only if no page produces meaningful text."},
	{Name: "OBJ_PERMISSIONS", Category: "Connection", Secret: false, Default: "client", Description: "Permissions for newly created tags and correspondents: `client`, `document`, or `none`."},
	{Name: "OLLAMA_CONTEXT_LENGTH", Category: "LLM", Secret: false, Default: "", Description: "(Ollama only) Positive integer that sets NumCtx (context window) for the Ollama runner. If unset or `0`, the model default is used."},
	{Name: "OLLAMA_HEADERS", Category: "LLM", Secret: true, Default: "", Description: "(Ollama only) Comma-separated `Key=Value` pairs added as HTTP headers to every Ollama request. Useful for authorization when Ollama is behind a reverse proxy (e.g. `Authorization=Bearer mytoken`)."},
	...
	{Name: "PAPERLESS_HTTP_TIMEOUT", Category: "Connection", Secret: false, Default: "5m", Description: "Timeout for Paperless API requests. Set to `0` to disable the client timeout."},
	...
	{Name: "VISION_LLM_HOST", Category: "OCR", Secret: false, Default: "", Description: "Legacy fallback URL for a separate vision Ollama service."},
